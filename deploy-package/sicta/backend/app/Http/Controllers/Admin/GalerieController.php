...
class GalerieController extends Controller {
    ...
    public function store(Request $request) {
        $data = $request->validate([
            ...
            'date' => 'nullable|date',
            'public' => 'boolean',
            'image_couverture' => 'nullable|image|max:20480',
            'medias'   => 'nullable|array|max:10',
            'medias.*' => 'nullable|file|mimes:jpg,jpeg,png,webp,mp4,mov|max:51200',
        ]);
        $data['public'] = $request->has('public') ? 1 : 0;
        if ($request->hasFile('image_couverture')) {
            $data['image_couverture'] = ImageUploadService::store($request->file('image_couverture'), 'galerie/covers');
